...
class GlfwBaseCallbackImpl : public GlfwBaseCallback {
 public:
  ...
  void OnGlfwDraw(GlfwBase *glfw) override {
    ...
    // set uniforms
    shader_single_color_.Use();
    glm::mat4 model = glm::mat4(1.0f);
    glm::mat4 view = camera.GetViewMatrix();
    glm::mat4 projection = camera.GetPerspectiveMatrix();
    shader_single_color_.SetMat4("view", view);
    shader_single_color_.SetMat4("projection", projection);
    ...
    shader_single_color_.SetMat4("model", model);
    glDrawArrays(GL_TRIANGLES, 0, 36);
    model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
    model = glm::scale(model, glm::vec3(scale, scale, scale));
