...
class GlfwBaseCallbackImpl : public GlfwBaseCallback {
 public:
  ...
  void OnGlfwDraw(GlfwBase *glfw) override {
    ...
    lighting_shader_.Use();
    lighting_shader_.SetVec3("light.position", camera.GetCamera().Position);
    lighting_shader_.SetVec3("light.direction", camera.GetCamera().Front);
    ...
    // light properties
    lighting_shader_.SetVec3("light.ambient", 0.1f, 0.1f, 0.1f);
    // we configure the diffuse intensity slightly higher; the right lighting conditions differ with each lighting method and environment.
    // each environment and lighting type requires some tweaking to get the best out of your environment.
    lighting_shader_.SetVec3("light.diffuse", 0.8f, 0.8f, 0.8f);
