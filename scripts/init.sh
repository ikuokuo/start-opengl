...
if [ "$HOST_OS" = "Linux" ]; then
  ...
  _install_deps "$SUDO apt-get install" libglew-dev libglfw3-dev libassimp-dev
elif [ "$HOST_OS" = "Mac" ]; then
  _install_deps "brew install" glew glfw assimp
elif [ "$HOST_OS" = "Win" ]; then
  if [ "$HOST_NAME" = "MINGW" ]; then
    if [ "$HOST_ARCH" = "x64" ]; then
      _install_deps "pacman -S" mingw-w64-x86_64-glew mingw-w64-x86_64-glfw mingw-w64-x86_64-assimp
    elif [ "$HOST_ARCH" = "x86" ]; then
      _install_deps "pacman -S" mingw-w64-i686-glew mingw-w64-i686-glfw mingw-w64-i686-assimp
    else
